...
import { IconPlus, IconPencil, IconTrash, IconTable, IconAlertTriangle, IconArrowsSort, IconArrowUp, IconArrowDown } from "@tabler/icons-react"

import { Level, columns } from "./columns"
import { translations, t } from "@/lib/translations"
import { Skeleton } from "@/components/ui/skeleton"
import { AUTH_TOKEN_KEY, type LevelPayload } from "@/lib/api"
...
interface LevelTableProps {
  ...
  hideActions?: boolean
  hideDelete?: boolean
  onCreate?: (payload: LevelPayload) => Promise<void>
  onUpdate?: (id: number, payload: Partial<LevelPayload>) => Promise<void>
...
const emptyFormData: LevelFormData = {
  id: "",
  school_id: "",
  name: "",
  description: "",
...
export function LevelTable({
  ...
  const [tableData, setTableData] = React.useState<Level[]>(initialData)
  const [open, setOpen] = React.useState(false)
  const [isEditing, setIsEditing] = React.useState(false)
  const [editingId, setEditingId] = React.useState<number | null>(null)
  const [isSaving, setIsSaving] = React.useState(false)
  const [formError, setFormError] = React.useState<string | null>(null)
  const [formData, setFormData] = React.useState<LevelFormData>(emptyFormData)
  ...
  React.useEffect(() => {
    if (searchQuery) {
      const filtered = initialData.filter((level) => {
        const searchLower = searchQuery.toLowerCase()
        return (
          level.name?.toLowerCase().includes(searchLower) ||
          level.status?.toLowerCase().includes(searchLower)
        )
      })
  ...
  const handleInputChange = (field: keyof LevelFormData, value: string) => {
  ...
  const handleEdit = (level: Level) => {
    ...
    setFormData({
      ...
      display_order: String(level.display_order),
      status: level.status,
    })
    setOpen(true)
  }
    ...
    try {
      ...
      if (isEditing && editingId !== null) {
        if (onUpdate) {
          ...
          if (!existing) {
            throw new Error("Level not found for local update")
          }
          const fallbackUpdated: Level = {
            ...existing,
  ...
  const actionColumn: ColumnDef<Level> = {
    id: "actions",
    cell: ({ row }) => {
    ...
    <div className="w-full space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          {table.getFilteredRowModel().rows.length} level(s)
        </div>
        <div className="flex items-center gap-2">
          {!readOnly && (
            <>
              <Button variant="outline" size="sm" className="h-8 w-8 rounded-full p-0 flex items-center justify-center border-emerald-600/30 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 text-emerald-600 hover:text-emerald-700" onClick={() => toast.success("Exporting to Excel...")}>
                  ...
                  <TabsContent value="informations">
                    <div className="grid gap-4 py-4">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Input id="name" placeholder="Enter level name" value={formData.name} onChange={(e) => handleInputChange("name", e.target.value)} />
                      </div>
                      <div className="grid gap-2">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Input id="display_order" type="number" placeholder="Enter order" value={formData.display_order} onChange={(e) => handleInputChange("display_order", e.target.value)} />
                      </div>
                    </div>
                  </TabsContent>
